    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4 bg-gradient-to-br from-blue-500/10 to-transparent border-white/5">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">P&L Total</p>
          <p className={`text-2xl font-black ${stats.totalPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
            ${(stats.totalPnL ?? 0).toFixed(2)}
          </p>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-blue-500/10 to-transparent border-white/5">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">Win Rate</p>
          <p className="text-2xl font-black text-blue-400">{stats.winRate}%</p>
        </Card>
        <Card className="p-4 bg-gradient-to-br from-blue-500/10 to-transparent border-white/5">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-1">P&L Promedio</p>
          <p className={`text-2xl font-black ${stats.avgPnL >= 0 ? 'text-green-400' : 'text-red-400'}`}>
      ...
      <Card className="p-4 bg-slate-900/40 border-white/5 shadow-inner backdrop-blur-xl">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="font-semibold text-white tracking-widest uppercase text-xs">Filtros</h3>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Símbolo</label>
            <input
              type="text"
              placeholder="BTC, ETH, SOL..."
              value={searchSymbol}
              onChange={(e) => setSearchSymbol(e.target.value)}
              className="w-full px-3 py-2 border border-white/10 rounded-lg bg-slate-950/50 backdrop-blur-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all"
            />
          </div>
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Mercado</label>
            <select
              value={filterMarket}
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Tipo</label>
            <select
              value={filterSide}
          ...
          <div>
            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Modo</label>
            <select
              value={filterMode}
